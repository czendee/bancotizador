...
//	var r = _r.PathPrefix("/v1").Subrouter()
	...
	r.Handle("/v3/processpayment", netHandle(handleDBProcesspayment, nil)).Methods("GET")           //logicbusiness.go
	...
	r.Handle("/v3/processpayment", netHandle(handleDBPostProcesspayment, nil)).Methods("POST")           //logicbusiness.go
...
// handleDBProcesspayment  receive and handle the request from client, access DB
func handleDBPostProcesspayment(w http.ResponseWriter, r *http.Request) {
	defer func() {
		db.Connection.Close(nil)
	}()
    var errorGeneral string
...
requestData,errorGeneral =obtainPostParmsProcessPayment(r,errorGeneral)  //logicrequest_post.go
    ...
    if errorGeneral=="" {
		...
		errorGeneral,errorGeneralNbr= ProcessProcessPayment(w , requestData)
	}
...
// handleDBProcesspayment  receive and handle the request from client, access DB
func handleDBProcesspayment(w http.ResponseWriter, r *http.Request) {
	defer func() {
		db.Connection.Close(nil)
	}()
